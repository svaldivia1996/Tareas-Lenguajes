...
<expr>   ::= <num>
           ...
           | {snd <expr>}
           | {if <expr> <expr> <expr>}
           | {with {{<id> <expr>}*} <expr>}  | {with { {<id> [: <type>] <expr>}* } <expr>}  ; los otros casos no cambian
           | {<id> <expr>*}
|#
(deftype Expr
    (Num n)
...
(test (parse '{define {add2 x} {+ 2 x}}) (fundef 'add2 (list 'x) (Add (Num 2) (Id 'x))));parseo de definicion de funciones con un arg
(test  (parse '{define {sum x y z} {+ x {+ y z}}}) (fundef 'sum (list 'x 'y 'z) (Add (Id 'x) (Add (Id 'y) (Id 'z)))));parseo de definicion de funciones con mult args
